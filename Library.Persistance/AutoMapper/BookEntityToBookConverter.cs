    ...
    public class BookEntityToBookConverter: ITypeConverter<BookEntity, Book> {
        public Book Convert( BookEntity source, Book destination, ResolutionContext context ) {
            if (source.BookType == BookType.Free) {
                return new FreeBook( source.Id, source.Title, source.Genre, source.Description, source.ISBN, source.Author );
            } else if (source.BookType == BookType.Taken) {
                return new TakenBook( source.Id, source.ClientId.Value, source.Title, source.Genre, source.Description, source.ISBN, source.Author, source.TakenAt.Value, source.ReturnTo.Value );
            }
